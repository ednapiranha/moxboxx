...
orm.connect('mysql://' + nconf.get('db_username') + ':' +
  ...
  exports.recentStarred = function(req, callback) {
    ...
    PlaylistStarred.find({ 'owner_id': req.session.userId }, 'id DESC', LIMIT, offset, function(starred) {
      var playlists = [];

      if (starred) {
        starred.forEach(function(star) {
          Playlist.get(star.playlist_id, function(p) {
  ...
  exports.getGlobal = function(req, callback) {
    ...
    Playlist.find('*', 'id DESC', LIMIT, offset, function(playlistCollection) {
      var playlists = [];
  ...
  exports.hasEditPermission = function(id, userId, callback) {
    Playlist.find({ 'id': id, 'user_id': userId }, function(p) {
      if (p) {
        callback(true);
      } else {
        callback(false);
  ...
  exports.userRecent = function(req, callback) {
    ...
    Playlist.find({ 'user_id': id }, 'id DESC', LIMIT, offset, function(playlistCollection) {
      var playlists = { data: [] };
  ...
  exports.yourRecent = function(req, callback) {
    var self = this;
    var id = parseInt(req.session.userId, 10);

    Playlist.find({ 'user_id': id }, 'title ASC', function(playlistCollection) {
      var playlists = [];
  ...
  exports.getRecentByTag = function(req, callback) {
    ...
    Tag.find({ name: tag }, function(tag) {
      if (tag) {
        ...
        PlaylistTags.find({ tags_id: tag[0].id }, LIMIT, offset, function(p) {
          if (p) {
            try {
