...
var sendNotificationStarred = function(req, id, nconf) {
  var fs = require('fs');
    ...
    where: {
      ...
      p.getUser().
        success(function(user) {
          if (user && user.email_starred && parseInt(user.id, 10) !== parseInt(req.session.userId, 10)) {
            console.log('sending to ', user.email)
            postmark.send({
              'From': 'moxboxx@noodleindustries.com',
              'To': user.email,
              'Subject': req.session.username + ' starred your playlist!',
              'HtmlBody': '<h1>Woo! Your playlist got starred!</h1> <p>' +
                req.session.username + ' starred ' + p.title +
                '. View it at <a href="http://moxboxx.com/playlist/' + p.id + '">' +
                'http://moxboxx.com/playlist/' + p.id + '</a></p>' +
                '<p>To stop receiving notifications, visit ' +
                '<a href="http://moxboxx.com/profile">http://moxboxx.com/profile</a> to update your settings.</p>',
              'TextBody': 'Woo! Your playlist got starred! ' + req.session.username +
                ' starred ' + p.title + '. ' + 'View it at http://moxboxx.com/playlist/' + p.id + '\n' +
                'To stop receiving notifications, visit http://moxboxx.com/profile to update your settings',
              'Attachments': [{
                'Content': fs.readFileSync(path.resolve('./public/images/starred.jpg')).toString('base64'),
                'Name': 'starred.jpg',
...
exports.star = function(req, nconf, callback) {
  ...
  PlaylistStarred.find({
    where: {
      ...
      if (!starred) {
        ...
        starred.save()
          .success(function(s) {
            sendNotificationStarred(req, id, nconf);
            callback(null, true);
          }).error(function(err) {
            callback(err);
          });
...
exports.addViewCount = function(req, playlist) {
  if (parseInt(req.session.userId, 10) !== parseInt(playlist.user_id, 10)) {
    playlist.id = parseInt(playlist.id, 10);
    playlist.views = parseInt(playlist.views, 10) + 1;
    playlist.save(['views']);
  }
};
