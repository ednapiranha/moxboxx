...
orm.connect('mysql://' + nconf.get('db_username') + ':' +
  ...
  exports.allByPlaylistId = function(id, isDeletable, callback) {
    ...
    Mox.find({ 'playlist_id': id }, 'pos ASC created ASC', function(moxCollection) {
      if (moxCollection) {
  ...
  exports.updatePositions = function(req) {
    ...
    if (sortArr) {
      playlist.hasEditPermission(sortArr[0].project_id, req.session.userId, function(resp) {
        if (resp) {
          sortArr.forEach(function(mox) {
            ...
            self.get(req, function(err, m) {
              if (!err) {
                ...
                editMox.save(function(err, mox) {
                  if (err) {
                    throw new Error('Could not save position for mox ', req.body.id);
                  }
                });
              }
            });
