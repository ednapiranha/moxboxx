...
var incrMox = function(req, media, client, callback) {
  client.incr('moxboxx:moxlistCount', function(err, id) {
    if (err) {
      ...
      client.rpush('moxboxx:playlist:moxes:' + req.body.playlist_id, id, function(err, moxIds) {
        if (err) {
          callback(err);
        } else {
          client.hmset('moxboxx:mox:' + id, mox, function(err, resp) {
            if (err) {
              callback(err);
            } else {
              callback(null, mox);
...
exports.get = function(req, client, callback) {
  client.hgetall('moxboxx:mox:' + req.params.id, function(err, m) {
    if (err) {
      callback(err);
    } else {
      if (m) {
...
exports.allByPlaylistId = function(req, client, callback) {
  ...
  client.lrange('moxboxx:playlist:moxes:' + req.params.id, 0, -1, function(err, moxIds) {
    if (err) {
      callback(err);
    } else {
      var moxList = [];
